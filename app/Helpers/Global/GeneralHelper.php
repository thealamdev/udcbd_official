...
if (!function_exists('bn_date')) {
  ...
  function bn_date()
  {
    ...
    $day = $numto->bnNum(now()->format('d'));
    $month = $numto->bnNum(now()->format('m'));
    $year = $numto->bnNum(now()->format('Y'));
    // for english to bangla
    $date_arr = [$day, $month, $year];
    $bangla_date = implode('-', $date_arr);
...
if (!function_exists('generate_order_number')) {
   ...
   * @return string
   */
  function generate_sanad_number($id, $length = 8)
  {
