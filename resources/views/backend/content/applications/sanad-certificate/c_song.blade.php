...
<div class="row">
              ...
              <div class="row mt-3 mb-5" style="margin-top: 2rem;">
                  ...
                  <div class="col-6 text-center border border-2 rounded-pill border-dark"
                      ...
                      <h6 style="font-size:25px;" class="m-2 language-switch" data-en="Amendment of National Identity Information" data-bn="জাতীয় পরিচয় তথ্য সংশোধন">জাতীয় পরিচয় তথ্য সংশোধন</h6>
                  </div>
              ...
              <!-- Row end -->
        <p style="font-size: 18.8px;letter-spacing: 1px;margin-top: 2rem; text-align:justify;" id="text" class="language-switch default-line-spacing" data-en="In this sense, the certificate of Amendment of National Identity Information is being issued, <strong>{{ $application->applicant }}</strong> National Identity Card / Birth Certificate No:  <strong>{{ $application->nid_birth }}</strong> Father/Husband:  <strong>{{ $application->father_husband_name }}, </strong>Village: <strong>{{ $application->mrgram }},</strong> Ward:   <strong>{{ $application->mrword }}</strong>, Post office: <strong>{{ $application->mrdak }}, </strong>
            </strong>Union:  <strong>{{ $application->union_name }}</strong>, Upazila: <strong>{{ $application->upazilla_name }}</strong>, District: <strong>{{ $application->zilla_name }}</strong> He belongs to my union: <strong>{{ $application->mrword }}</strong> A permanent resident of Ward no. I know him personally. He is not involved in any anti-social or anti-state activities. He is a permanent citizen and resident of Union. As far as I know, his name/date of birth has been mistakenly printed as __________ instead of __________ in his National Identity Card. Therefore, I recommend to correct his/her National Identity Card." data-bn="   এই মর্মে জাতীয় পরিচয় তথ্য সংশোধন প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
           ...
           নং ওয়ার্ডের একজন স্থায়ী বাসিন্দা। আমি তাকে ব্যক্তি গত ভাবে চিনি ও জানি। সে সমাজ বা রাষ্ট্র বিরোধী কোন প্রকার কাজের সহিত জড়িত নাই। সে অত্র ইউনিয়ন এর স্থায়ী নাগরিক ও বাসিন্দা। আমার জানা মতে তার জাতীয় পরিচয়পত্রে নাম/ জন্ম তারিখ ভুল বশত ___________ এর স্থলে__________মুদ্রীত হয়েছে। বিধায় তার_________জাতীয় পরিচয় পত্র সংশোধন করার জন্য সুপারিশ করছি।">
           এই মর্মে জাতীয় পরিচয় তথ্য সংশোধন প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
