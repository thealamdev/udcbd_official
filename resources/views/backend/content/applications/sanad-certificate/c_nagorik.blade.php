...
<div class="row">
        ...
        <p style="font-size: 18.8px;letter-spacing: 1px;margin-top: 2rem; text-align:justify;" id="text" class="language-switch default-line-spacing" data-en="In this sense, the certificate of Citizenship is being issued, <strong>{{ $application->applicant }}</strong> National Identity Card / Birth Certificate No:  <strong>{{ $application->nid_birth }}</strong> Father/Husband:  <strong>{{ $application->father_husband_name }}, </strong>Village: <strong>{{ $application->mrgram }},</strong> Ward:   <strong>{{ $application->mrword }}</strong>, Post office: <strong>{{ $application->mrdak }}, </strong>
           ...
           এই মর্মে  নাগরিক সনদ প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
