...
<div class="row">
              ...
              <div class="row mt-3 mb-5" style="margin-top: 2rem;">
                  ...
                  <div class="col-6 text-center border border-2 rounded-pill border-dark"
                      ...
                      <h6 style="font-size:25px;" class="m-2 language-switch" data-en="Dead certificate" data-bn="মৃত্যু  সনদ">মৃত্যু  সনদ</h6>
                  </div>
              ...
              <!-- Row end -->
        <p style="font-size: 18.8px;letter-spacing: 1px;margin-top: 2rem; text-align:justify;" id="text" class="language-switch default-line-spacing" data-en="In this sense, the certificate of Dead is being issued, <strong>{{ $application->applicant }}</strong> National Identity Card / Birth Certificate No:  <strong>{{ $application->nid_birth }}</strong> Father/Husband:  <strong>{{ $application->father_husband_name }}, </strong>Village: <strong>{{ $application->mrgram }},</strong> Ward:   <strong>{{ $application->mrword }}</strong>, Post office: <strong>{{ $application->mrdak }}, </strong>
            </strong>, Union:  <strong>{{ $application->union_name }}</strong>, Upazila: <strong>{{ $application->upazilla_name }}</strong>, District: <strong>{{ $application->zilla_name }}</strong> He belongs to my union: <strong>{{ $application->mrword }}</strong> A permanent resident of Ward no.As far as I know he died on <strong>{{ $application->death_date }}</strong>." data-bn="   এই মর্মে মৃত্যু  সনদ প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
           ...
           নং ওয়ার্ডের একজন স্থায়ী বাসিন্দা। আমার জানা মতে তিনি <strong>{{ $application->death_date }}</strong> ইং তারিখে মৃত্যু বরন করেছে।।।">
           এই মর্মে  মৃত্যু  সনদ প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
...
">
আমি তার  বিদেহী আত্মার মাগফিরাত/শান্তি কামনা  করি।
</p>
