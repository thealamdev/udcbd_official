...
<div class="row">
              ...
              <div class="row mt-3 mb-5" style="margin-top: 2rem;">
                  ...
                  <div class="col-6 text-center border border-2 rounded-pill border-dark"
                      ...
                      <h6 style="font-size:25px;" class="m-2 language-switch" data-en="Parent's permission" data-bn="অভিভাবকের অনুমতিপত্র">অভিভাবকের অনুমতিপত্র</h6>
                  </div>
              ...
              <!-- Row end -->
        <p style="font-size: 18.8px;letter-spacing: 1px;margin-top: 2rem; text-align:justify;" id="text" class="language-switch default-line-spacing" data-en="In this sense, the certificate of Parent's permission is being issued, <strong>{{ $application->applicant }}</strong> National Identity Card / Birth Certificate No:  <strong>{{ $application->nid_birth }}</strong> Father/Husband:  <strong>{{ $application->father_husband_name }}, </strong>Village: <strong>{{ $application->mrgram }},</strong> Ward:   <strong>{{ $application->mrword }}</strong>, Post office: <strong>{{ $application->mrdak }}, </strong>
            </strong>, Union:  <strong>{{ $application->union_name }}</strong>, Upazila: <strong>{{ $application->upazilla_name }}</strong>, District: <strong>{{ $application->zilla_name }}</strong> I hereby declare that my son/daughter <strong>{{ $application->celer }}</strong> | Because of that bravery and morale, I gave permission in my right mind and consciousness to join the Bangladesh  <strong>{{ $application->bahinir }}</strong> Bahini and employ myself in the defense of the country. " data-bn=" আমি, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
            </strong>ইউনিয়নেঃ <strong>{{ $application->union_name }}</strong>, উপজেলাঃ <strong>{{ $application->upazilla_name }}</strong>, জেলাঃ <strong>{{ $application->zilla_name }}</strong> এই মর্মে আঙ্গিকার করেতিছি যে, আমার পুত্র/কন্যা <strong>{{ $application->celer }}</strong> | সে সাহসিকতা ও মনোবলে বলিয়ান হেতু বাংলাদেশ  <strong>{{ $application->bahinir }}</strong> বাহীনিতে যোগ দিয়া দেশ রক্ষার কাজে আত্ম নিয়োগ করার জন্য  আমি সুস্থ মস্তিষ্কে এবং সজ্ঞানে অনুমতি প্রদান করিলাম।।">
           আমি, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
            </strong>ইউনিয়নঃ <strong>{{ $application->union_name }}</strong>, উপজেলাঃ <strong>{{ $application->upazilla_name }}</strong>, জেলাঃ <strong>{{ $application->zilla_name }}</strong>   এই মর্মে আঙ্গিকার করেতিছি যে, আমার পুত্র/কন্যা <strong>{{ $application->celer }}</strong> | সে সাহসিকতা ও মনোবলে বলিয়ান হেতু বাংলাদেশ  <strong>{{ $application->bahinir }}</strong> বাহীনিতে যোগ দিয়া দেশ রক্ষার কাজে আত্ম নিয়োগ করার  জন্য  আমি সুস্থ মস্তিষ্কে এবং সজ্ঞানে অনুমতি প্রদান করিলাম।
...
আমি তার জীবনের সার্বিক উন্নতি ও মঙ্গল কামনা করি।
              ...
              <div class="col-3 text-center justify-content-center align-items-center mt-4">
                      <hr style="border: 1px solid black;margin-top:6rem; width:90%">
                      <p class="font-6 language-switch" data-en="Signature of guardian" data-bn="অভিভাবকের স্বাক্ষর">অভিভাবকের স্বাক্ষর</p>
                  </div>
              ...
              <div class="row mb-4 print-mb-2" style="margin-top: 2em;">
                  ...
                  <div class="col-3 d-flex align-items-center">
                      <div class="text-center border border-1 seal-border">
                          <p class="mt-3 pt-4 font-6 language-switch" data-en="Seal of office" data-bn="কার্যালয়ের সীল"> কার্যালয়ের গোল সীল </p>
                      </div>
                  </div>
