@if ($user->trashed() && $logged_in_user->hasAllAccess())
    <x-utils.form-button :action="route('admin.auth.user.restore', $user)" method="patch" button-class="btn btn-info btn-sm" icon="fas fa-sync-alt"
        name="confirm-item">
        @lang('Restore')
    </x-utils.form-button>

    @if (config('boilerplate.access.user.permanently_delete'))
        <x-utils.delete-button :href="route('admin.auth.user.permanently-delete', $user)" :text="__('Permanently Delete')" />
    @endif
@else
    ...
    @if ($user->isMasterAdmin() && $logged_in_user->isMasterAdmin())
        <div class="dropdown d-inline-block">
            ...
            <div class="dropdown-menu" aria-labelledby="moreMenuLink">
                <x-utils.link :href="route('admin.auth.user.change-password', $user)" class="dropdown-item" :text="__('Change Password')"
                    permission="admin.access.user.change-password" />

                @if ($logged_in_user->hasAllAccess())
                    <li class="nav-item ml-3">
                        <a href="{{ route('admin.setting.rechargeBalance', ['id' => $user]) }}">Rechage Account</a>
                    </li>
                @endif
            </div>
        </div>
        ...
        <div class="dropdown d-inline-block">
            ...
            <div class="dropdown-menu" aria-labelledby="moreMenuLink">
                ...
                @if ($user->id !== $logged_in_user->id && !$user->isMasterAdmin())
                    <x-utils.form-button :action="route('admin.auth.user.clear-session', $user)" name="confirm-item" button-class="dropdown-item"
                        permission="admin.access.user.clear-session">
                        @lang('Clear Session')
                    </x-utils.form-button>

                    @canBeImpersonated($user)
                    <x-utils.link :href="route('impersonate', $user->id)" class="dropdown-item" :text="__('Login As ' . $user->name)"
                        permission="admin.access.user.impersonate" />
                    @endCanBeImpersonated

                    <x-utils.form-button :action="route('admin.auth.user.mark', [$user, 0])" method="patch" name="confirm-item"
                        button-class="dropdown-item" permission="admin.access.user.deactivate">
                        @lang('Deactivate')
                    </x-utils.form-button>
                @endif
            </div>
