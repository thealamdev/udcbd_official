...
<div class="row">
        ...
        <p style="font-size: 18.8px;letter-spacing: 1px;margin-top: -2rem; text-align:justify;" id="text" class="language-switch default-line-spacing" data-en="In this sense, the certificate of Warishian  is being issued, <strong>{{ $application->applicant }}</strong> National Identity Card / Birth Certificate No:  <strong>{{ $application->nid_birth }}</strong> Father/Husband:  <strong>{{ $application->father_husband_name }}, </strong>Village: <strong>{{ $application->mrgram }},</strong> Ward:   <strong>{{ $application->mrword }}</strong>, Post office: <strong>{{ $application->mrdak }}, </strong>
            </strong>, Union:  <strong>{{ $application->union_name }}</strong>, Upazila: <strong>{{ $application->upazilla_name }}</strong>, District: <strong>{{ $application->zilla_name }}</strong> He belongs to my union: <strong>{{ $application->mrword }}</strong> A permanent resident of Ward no. According to the informant, he died as underwritten Warishian. " data-bn="   এই মর্মে ওয়ারিশিয়া   সনদপত্র প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
           ...
           এই মর্মে ওয়ারিশিয়ান   সনদপত্র প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / মৃত্যু সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
              ...
              <table class="table table-bordered table-responsive font-12">
                  ...
                  <tbody>
                      @foreach ($sl as $key => $val)
                          <tr>
                              <td class="text-center" style="font-size: 18.8px;" >{{ $sl[$key] }}</td>
                              <td class="text-center"  style="font-size: 18.8px;" >{{ $name[$key] }}</td>
                              <td class="text-center" style="font-size: 18.8px;" >{{ $rel[$key] }}</td>
                        
                              <td class="text-center" style="font-size: 18.8px;" >{{ $enid[$key] }}</td>
                                    <td class="text-center" style="font-size: 18.8px;" >{{ $ebirth[$key] }}</td>
                              <td class="text-center" style="font-size: 18.8px;" >{{ $ecom[$key] }}</td>
