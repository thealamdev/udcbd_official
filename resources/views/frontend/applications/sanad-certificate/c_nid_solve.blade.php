...
<div class="row">
              ...
              <div class="row mt-3 mb-5" style="margin-top: 2rem;">
                  ...
                  <div class="col-6 text-center border border-2 rounded-pill border-dark"
                      ...
                      <h6 style="font-size:25px;" class="m-2 language-switch" data-en="Certificate" data-bn="প্রত্যয়ন পত্র">প্রত্যয়ন পত্র</h6>
                  </div>
              ...
              <!-- Row end -->
        <p style="font-size: 18.8px;letter-spacing: 1px;margin-top: 2rem; text-align:justify;" id="text" class="language-switch default-line-spacing" data-en="In this sense, the certificate of Voter Area Transfer is being issued, <strong>{{ $application->applicant }}</strong> National Identity Card / Birth Certificate No:  <strong>{{ $application->nid_birth }}</strong> Father/Husband:  <strong>{{ $application->father_husband_name }}, </strong>Village: <strong>{{ $application->mrgram }},</strong> Ward:   <strong>{{ $application->mrword }}</strong>, Post office: <strong>{{ $application->mrdak }}, </strong>
            </strong>, Union:  <strong>{{ $application->union_name }}</strong>, Upazila: <strong>{{ $application->upazilla_name }}</strong>, District: <strong>{{ $application->zilla_name }}</strong> He belongs to my union: <strong>{{ $application->mrword }}</strong> A permanent resident of Ward no. I know him personally. He is not involved in any anti-social or anti-state activities. According to my knowledge he is in <strong>{{ $application->present_zila }}</strong>district <strong>{{ $application->present_upazila }}</strong> upazila <strong>{{ $application->present_village }}</strong> His name is included in the electoral roll at the address mentioned in village. He is currently your village: <strong>{{ $application->transfer_village }}</strong> Post Office: <strong>{{ $application->transfer_basha }}</strong> Upazila <strong>{{ $application->transfer_upazila }}</strong> Zilla <strong>{{ $application->transfer_zila }}</strong> Resident for long time. So want to transfer his voter to present address . " data-bn="এই মর্মে ভোটার এলাকা স্থানান্তর প্রত্যয়ন প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
           ...
           নং ওয়ার্ডের একজন স্থায়ী বাসিন্দা। আমি তাকে ব্যক্তি গত ভাবে চিনি ও জানি। সে সমাজ বা রাষ্ট্র বিরোধী কোন প্রকার কাজের সহিত জড়িত নাই। আমার জানামতে সে <strong>{{ $application->present_zila }}</strong>জেলার <strong>{{ $application->present_upazila }}</strong> উপজেলায় <strong>{{ $application->present_village }}</strong> গ্রামে উল্লেখিত ঠিকানায় ভোটার তালিকায় তার নাম অন্তর্ভুক্ত আছে।বর্তমানে  তিনি গ্রাম:  <strong>{{ $application->transfer_village }}</strong> ডাকঘরঃ <strong>{{ $application->transfer_basha }}</strong>  উপজেলা <strong>{{ $application->transfer_upazila }}</strong> জেলা <strong>{{ $application->transfer_zila }}</strong> দীর্ঘদিন ধরে বসবাস করে।তাই বর্তমান ঠিকানায় তার ভোটার স্থানান্তর করতে ইচ্ছুক।
।">
           এই মর্মে ভোটার এলাকা স্থানান্তর প্রত্যয়ন প্রদান করা যাইতেছে যে, <strong>{{ $application->applicant }}</strong> জাতীয় পরিচয়পত্র / জন্ম সনদ নং: <strong>{{ $application->nid_birth }}</strong> পিতা/স্বামীঃ <strong>{{ $application->father_husband_name }}, </strong>গ্রামঃ<strong>{{ $application->mrgram }},</strong> ওয়ার্ডঃ <strong>{{ $application->mrword }}</strong>, ডাকঘরঃ<strong>{{ $application->mrdak }}, </strong>
