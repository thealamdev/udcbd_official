...
use App\Http\Controllers\Agent\Client\ClientAuthController;
...
Route::group(['as' => 'auth.'], function () {
    ...
    /**
     * routes for agents client registration
     */
    Route::group(['middleware' => 'guest'], function () {
        Route::prefix('clients')->name('client.')->group(function () {
            Route::get('register', [ClientAuthController::class, 'register'])->name('register');
            Route::post('registered', [ClientAuthController::class, 'registered'])->name('registered');
        });

        Route::prefix('agents')->name('agent.')->group(function () {
            Route::get('register', [AgentAuthController::class, 'register'])->name('register');
            Route::post('registered', [AgentAuthController::class, 'registered'])->name('registered');
        });
    });
});
